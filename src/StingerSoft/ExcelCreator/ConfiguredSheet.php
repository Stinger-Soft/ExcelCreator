...
class ConfiguredSheet {
	 ...
	 * @var string
	 */
	protected $defaultHeaderBackgroundColor = 'B8<?php
...
class ConfiguredSheet {
	...
	protected function renderDataCell(\PHPExcel_Cell &$cell, $item, ColumnBinding $binding, array $extraData) {
		...
		$link = $this->getPropertyFromObject($item, $binding, $binding->getDataStyling(), null, $extraData);
		...
		$cell->setValue($value);
		if($binding->getLinkUrl() != null) {
			if(is_callable($binding->getLinkUrl())) {
				$url = call_user_func_array($binding->getLinkUrl(), array(
					$binding,
					$item,
					$extraData
				));
